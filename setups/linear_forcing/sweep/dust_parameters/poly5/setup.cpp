...
static real cs;
real epsilon;
const int ns=3;   // number of dust species. compile time at the moment but will change later to run time
real chi[ns];
real tauGlob[ns];
real chi_tot;
real pi=3.14;
...
// Mirror data on Host
  ...
  // Sync it
  d.SyncFromDevice();
  real rho = d.dustVc[0](RHO,0,0,0)+d.dustVc[1](RHO,0,0,0)+d.dustVc[2](RHO,0,0,0);
  real KE = 0.0;
  real rho_i = 0.0;
  for(int k = 0; k < d.np_tot[KDIR] ; k++) {
    for(int j = 0; j < d.np_tot[JDIR] ; j++) {
      for(int i = 0; i < d.np_tot[IDIR] ; i++) {
        ...
        rho_i = d.dustVc[0](RHO,k,j,i)+d.dustVc[1](RHO,k,j,i)+d.dustVc[2](RHO,k,j,i);
        if(rho < rho_i) {
          rho=rho_i;
        }
      }
...
Setup::Setup(Input &input, Grid &grid, DataBlock &data, Output &output) {
  ...
  data.dust[1]->EnrollUserDefBoundary(&reversePeriodicDust);
  data.dust[2]->EnrollUserDefBoundary(&reversePeriodicDust);
    ...
    for(int k = 0; k < d.np_tot[KDIR] ; k++) {
        for(int j = 0; j < d.np_tot[JDIR] ; j++) {
            for(int i = 0; i < d.np_tot[IDIR] ; i++) {
                ...
                d.dustVc[1](RHO,k,j,i) = chi[1];
                // Equations 7c-7d of Youdin & Johansen (2007) with eta = epsilon/(2Omega Vk)
                d.dustVc[1](VX1,k,j,i) = 4.1609302*z+2e-3*(idfx::randm()-0.5);
                d.dustVc[1](VX2,k,j,i) = -0.0129259*z;

                //d.dustVc[0](VX1,k,j,i) += 0*1e-1*(idfx::randm()-0.5);
                d.dustVc[1](VX2,k,j,i) += shear*x+2e-3*(idfx::randm()-0.5);
                d.dustVc[1](VX3,k,j,i) = 0.0+2e-3*(idfx::randm()-0.5);

                d.dustVc[2](RHO,k,j,i) = chi[2];
                // Equations 7c-7d of Youdin & Johansen (2007) with eta = epsilon/(2Omega Vk)
                d.dustVc[2](VX1,k,j,i) = 4.1609302*z+2e-3*(idfx::randm()-0.5);
                d.dustVc[2](VX2,k,j,i) = -0.0129259*z;

                //d.dustVc[0](VX1,k,j,i) += 0*1e-1*(idfx::randm()-0.5);
                d.dustVc[2](VX2,k,j,i) += shear*x+2e-3*(idfx::randm()-0.5);
                d.dustVc[2](VX3,k,j,i) = 0.0+2e-3*(idfx::randm()-0.5);
