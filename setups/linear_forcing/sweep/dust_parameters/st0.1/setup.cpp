    ...
    for(int k = 0; k < d.np_tot[KDIR] ; k++) {
        for(int j = 0; j < d.np_tot[JDIR] ; j++) {
            for(int i = 0; i < d.np_tot[IDIR] ; i++) {
                ...
                d.Vc(VX1,k,j,i) = 4.145824415*z+2e-3*(idfx::randm()-0.5);
                //d.Vc(VX1,k,j,i) += 1.*psi*z*omega;
                d.Vc(VX2,k,j,i) = 9.57041e-6*z;
                ...
                d.dustVc[0](VX1,k,j,i) = 4.1464998*z+2e-3*(idfx::randm()-0.5);
                d.dustVc[0](VX2,k,j,i) = -4.7852e-5*z;
