...
void InducedWarpFlow(Hydro *hydro, const real t, const real dt) {
  ...
  idefix_for("InducedWarpFlow",0,data->np_tot[KDIR],0,data->np_tot[JDIR],0,data->np_tot[IDIR],
              KOKKOS_LAMBDA (int k, int j, int i) {
                  Uc(MX1,k,j,i) += Vc(RHO,k,j,i)*omegaLocal*omegaLocal*H*psiLocal*sin(2.0*pi*z(k)/Lz)*cos(omegaLocal*t)*dt;
              });
}
    ...
    for(int k = 0; k < d.np_tot[KDIR] ; k++) {
        for(int j = 0; j < d.np_tot[JDIR] ; j++) {
            for(int i = 0; i < d.np_tot[IDIR] ; i++) {
                ...
                d.Vc(RHO,k,j,i) = 1.0+2e-2*(idfx::randm()-0.5);
                // Equations 7a-7b of Youdin & Johansen (2007) with eta = epsilon/(2Omega Vk)
                d.Vc(VX1,k,j,i) = 0.0*sin(2.0*pi*z);
                //d.Vc(VX1,k,j,i) += 1.*psi*z*omega;
                d.Vc(VX2,k,j,i) = 0.0*sin(2.0*pi*z);
                d.Vc(VX1,k,j,i) += 2e-2*cs*(idfx::randm()-0.5);
                d.Vc(VX2,k,j,i) += shear*x+2e-2*cs*(idfx::randm()-0.5);
                d.Vc(VX3,k,j,i) = 0.0+2e-2*cs*(idfx::randm()-0.5);

                d.dustVc[0](RHO,k,j,i) = chi+2e-2*(idfx::randm()-0.5);
                // Equations 7c-7d of Youdin & Johansen (2007) with eta = epsilon/(2Omega Vk)
                d.dustVc[0](VX1,k,j,i) = 0.0*sin(2.0*pi*z);
                d.dustVc[0](VX2,k,j,i) = 0.0*sin(2.0*pi*z);

                d.dustVc[0](VX1,k,j,i) += 2e-2*cs*(idfx::randm()-0.5);
                d.dustVc[0](VX2,k,j,i) += shear*x+2e-2*cs*(idfx::randm()-0.5);
