...
real tauGlob;
real pi=3.14;
real max_ug = 0.828587666908496;
real max_vg = 0.4232139129382706;
real max_ud = 0.8322097529188573;
real max_vd = 0.4213585838015952;
...
void nonReflective(DataBlock& data, int dir, BoundarySide side, real t) {
  ...
  real shearLocal = shear;
  real chilocal = chi;
  real max_ug_local = max_ug;
  real max_vg_local = max_vg;
  real max_ud_local = max_ud;
  real max_vd_local = max_vd;
    ...
    KOKKOS_LAMBDA (int k, int j, int i) {
      int iref, jref, kref;
        ...
        if(kref == k){
          ...
          Vc(VX1,k,j,i) = max_ug_local*z(k)*cos(t);
          Vc(VX2,k,j,i) = -max_vg_local*z(k)*sin(t)+shearLocal*x(i);
        }
        });
    ...
    KOKKOS_LAMBDA (int k, int j, int i) {
      int iref, jref, kref;
        ...
        if(kref == k){
          ...
          Uc(VX1,k,j,i) = max_ud_local*z(k)*cos(t);
          Uc(VX2,k,j,i) = -max_vd_local*z(k)*sin(t)+shearLocal*x(i);
        }
...
void nonReflective_non_periodic(DataBlock& data, int dir, BoundarySide side, real t) {
  idfx::pushRegion("nonReflective_non_periodic");
  IdefixArray4D<real> Vc = data.hydro->Vc;
  IdefixArray4D<real> Uc = data.dust[0]->Vc;
  ...
  data.hydro->boundary->BoundaryFor("nonReflective_non_periodic", dir, side,
    KOKKOS_LAMBDA (int k, int j, int i) {
      int iref, jref, kref;
        // This hack takes care of cases where we have more ghost zones than active zones
        //real x=d.x[IDIR](i);
  ...
  data.dust[0]->boundary->BoundaryFor("nonReflective_non_periodic", dir, side,
    KOKKOS_LAMBDA (int k, int j, int i) {
      int iref, jref, kref;
        // This hack takes care of cases where we have more ghost zones than active zones
        //real x=d.x[IDIR](i);
...
void extrapolate(DataBlock& data, int dir, BoundarySide side, real t) {
  idfx::pushRegion("extrapolate");
  IdefixArray4D<real> Vc = data.hydro->Vc;
  IdefixArray4D<real> Uc = data.dust[0]->Vc;
  ...
  data.hydro->boundary->BoundaryFor("extrapolate", dir, side,
    KOKKOS_LAMBDA (int k, int j, int i) {
      int iref, jref, kref;
        const int kactive = (dir==KDIR) ? kghost + side*(nxk-1) : k;
        ...
        if(kref == k){
          ...
          Vc(VX1,k,j,i) = Vc(VX1,kactive,jref,iref)+max_ug_local*(z(k)-z(kactive))*cos(t);
          Vc(VX2,k,j,i) = Vc(VX2,kactive,jref,iref)-max_vg_local*(z(k)-z(kactive))*sin(t);
        }
        });
  data.dust[0]->boundary->BoundaryFor("extrpolate", dir, side,
    KOKKOS_LAMBDA (int k, int j, int i) {
      int iref, jref, kref;
        const int kactive = (dir==KDIR) ? kghost + side*(nxk-1) : k;
        ...
        if(kref == k){
          ...
          Uc(VX1,k,j,i) = Uc(VX1,kactive,jref,iref)+max_ud_local*(z(k)-z(kactive))*cos(t);
          Uc(VX2,k,j,i) = Uc(VX2,kactive,jref,iref)-max_vd_local*(z(k)-z(kactive))*sin(t);
        }
...
Setup::Setup(Input &input, Grid &grid, DataBlock &data, Output &output) {
  ...
  //data.hydro->EnrollUserDefBoundary(reversePeriodic);
  //data.dust[0]->EnrollUserDefBoundary(reversePeriodic);
  //data.hydro->EnrollUserDefBoundary(nonReflective);
  //data.dust[0]->EnrollUserDefBoundary(nonReflective);
  //data.hydro->EnrollUserDefBoundary(extrapolate);
  //data.dust[0]->EnrollUserDefBoundary(extrapolate);
  //data.hydro->EnrollUserDefBoundary(nonReflective_non_periodic);
  //data.dust[0]->EnrollUserDefBoundary(nonReflective_non_periodic);
    ...
    for(int k = 0; k < d.np_tot[KDIR] ; k++) {
        for(int j = 0; j < d.np_tot[JDIR] ; j++) {
            for(int i = 0; i < d.np_tot[IDIR] ; i++) {
                ...
                d.Vc(VX1,k,j,i) = 0.828587666908496*z+2e-3*(idfx::randm()-0.5);
                //d.Vc(VX1,k,j,i) += 1.*psi*z*omega;
                d.Vc(VX2,k,j,i) = 0.000517*z;
                ...
                d.dustVc[0](VX1,k,j,i) = 0.8321860435*z+2e-3*(idfx::randm()-0.5);
                d.dustVc[0](VX2,k,j,i) = -0.00258518*z;
