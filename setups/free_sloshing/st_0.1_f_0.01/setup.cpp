    ...
    for(int k = 0; k < d.np_tot[KDIR] ; k++) {
        for(int j = 0; j < d.np_tot[JDIR] ; j++) {
            for(int i = 0; i < d.np_tot[IDIR] ; i++) {
                ...
                d.dustVc[0](RHO,k,j,i) *= exp(-0.00761230*cos(Kx*x+Kb*1*z)-0.00202237*sin(Kx*x+Kb*1*z));
                d.dustVc[0](RHO,k,j,i) *= exp(0.00493997*cos(Kx*x+Kb*2*z)-0.00020321*sin(Kx*x+Kb*2*z));
                ...
                d.dustVc[0](VX2,k,j,i) += 0.00002830*cos(Kx*x+Kb*1*z)-0.00012541*sin(Kx*x+Kb*1*z);
                d.dustVc[0](VX2,k,j,i) += 0.00000736*cos(Kx*x+Kb*2*z)+0.00007811*sin(Kx*x+Kb*2*z);

                d.dustVc[0](VX3,k,j,i) = 0.0;
                d.dustVc[0](VX3,k,j,i) += 0.00020810*cos(Kx*x+Kb*1*z)+0.00011246*sin(Kx*x+Kb*1*z);
                d.dustVc[0](VX3,k,j,i) += 0.00012298*cos(Kx*x+Kb*2*z)-0.00000247*sin(Kx*x+Kb*2*z);
